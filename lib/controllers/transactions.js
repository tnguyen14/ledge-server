 ...
 *
 */
function removeMerchantFromCounts(merchant, counts) {
  var slug = slugify(merchant);
  var _counts = counts;
...
function addMerchant(merchant, account) {
  var slug = slugify(merchant);
  return DB.collection('accounts').findOne({name: account}).then(function (account) {
    return DB.collection('accounts').update({name: account}, {
      $set: {
        merchants_count: addMerchantToCounts(merchant, account.merchants_count)
      }
    });
  });
}
...
function updateMerchant(newMerchant, oldMerchant, account) {
  var newSlug = slugify(newMerchant);
  var oldSlug = slugify(oldMerchant);
  ...
  return DB.collection('accounts').findOne({name: account}).then(function (account) {
    var merchantsCount = account.merchants_count;
    // if the slugs are the same but the name is not, add to the values array
    if (newSlug === oldSlug) {
      merchantsCount[newSlug].values.push(newMerchant);
    // otherwise, remove old one, add new one;
    } else {
      merchantsCount = removeMerchantFromCounts(oldMerchant, merchantsCount);
...
function removeMerchant(merchant, account) {
  ...
  return DB.collection('accounts').findOne({name: account}).then(function (account) {
    return DB.collection('accounts').update({name: account}, {
      $set: {
        merchants_count: removeMerchantFromCounts(merchant, account.merchants_count)
      }
    });
  });
}
...
function newTransaction(params) {
  ...
  var time = (params.time) ? params.time : '08:00'; // default to 08:00 AM
  var date = (params.date) ? new Date(params.date + ' ' + time) : new Date();
  ...
  addMerchant(merchant, params.name);
...
function deleteTransaction(params) {
  DB.collection(params.name).findOne({_id: params._id}, {_id: 0}).then(function (transaction) {

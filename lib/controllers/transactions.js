...
function getUniqueTransactionId (id, accountName, callback) {
	...
	async.until(
		...
		function (cb) {
			db.get(['transaction', accountName, _id].join(SEPARATOR), function (err) {
				if (err && err.notFound) {
					notFound = true;
				} else {
					_id++;
...
function showWeekly (params, callback) {
	...
	var thisMonday = moment().tz(timezone).isoWeekday(1 + dayOffset).startOf('day').valueOf();
	var nextMonday = moment().tz(timezone).isoWeekday(8 + dayOffset).startOf('day').valueOf();
	db.getRange({
...
function showOne (params, callback) {
	...
	db.get('transaction!' + params.name + '!' + params.id, function (err, transaction) {
		if (err) {
			if (err.notFound) {
				var transactionNotFound = new Error('No such transaction was found.');
...
function newTransaction (params, callback) {
	...
	if (!params.amount) {
		let missingAmount = new Error('Cannot add a transaction without amount');
		missingAmount.status = 409;
		return callback(missingAmount);
	}
	if (!params.date || !params.time) {
		let missingDateTime = new Error('Cannot add a transaction without date and time');
		return callback(missingDateTime);
	}
	var date = moment.tz(params.date + ' ' + (params.time || '08:00'), timezone);
	var id = date.valueOf();
	var uniqueId;
	...
	async.series([
		...
		function (cb) {
			db.put(['transaction', params.name, uniqueId].join(SEPARATOR), {
...
function updateTransaction (params, callback) {
	...
	var oldTransactionId = ['transaction', params.name, params.id].join(SEPARATOR);
	// opts is a conditional subset of params with some parsing
	var opts = {};
	var newTransaction, oldTransaction, uniqueId, newTransactionId, date, hasDateChange;
	if (params.date && params.time) {
		date = moment.tz(params.date + ' ' + (params.time || '08:00'), timezone);
	...
	async.series([
		...
		function (cb) {
			...
			getUniqueTransactionId(date.valueOf(), params.name, function (err, id) {
				if (!err) {
					uniqueId = id;
					newTransactionId = ['transaction', params.name, uniqueId].join(SEPARATOR);
				}
				cb(null);
			});
		},
		...
		function (cb) {
			db.put(newTransactionId, Object.assign({}, oldTransaction, newTransaction), cb);
		},
		function (cb) {

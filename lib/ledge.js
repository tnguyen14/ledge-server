...
server.put('/accounts/:name/transactions/:_id', function (req, res, next) {
  MongoClient.connect(DBURL, function (err, db) {
    ...
    if (req.params.date) {
      req.params.date = moment(new Date(req.params.date)).toDate();
    }
    var account = db.collection(req.params.name);
    account.findOne({_id: transactionId}, function (err2, transaction) {
      if (err2) {db.close(); return next(err);}
      var amount = req.params.amount || transaction.amount,
        date = req.params.date || transaction.date,
        desc = req.params.description || transaction.description,
        cat = req.params.category || transaction.category;
        account.update({_id: transactionId}, {
          amount: amount,
          date: date,
          description: desc,
          category: cat
        }, {w: 1}, function (err3, result) {
